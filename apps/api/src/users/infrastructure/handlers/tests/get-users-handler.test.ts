...
test("sends 200 with a list of users", async () => {
  ...
  await request(app).get("/").query({ username: "test" }).expect(200).expect(expected)
})

test("sends 200 with an empty list when no users match", async () => {
  repo.search.mockResolvedValueOnce([])

  await request(app).get("/").query({ firstName: "nonexistent" }).expect(200).expect([])
})
...
test("sends 200 with pagination options", async () => {
  ...
  await request(app).get("/").query({ username: "test", limit: "10", offset: "0" }).expect(200)
})
