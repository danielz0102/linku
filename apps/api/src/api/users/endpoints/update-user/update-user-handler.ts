...
import type { UpdateUserError, UpdateUser } from "#core/use-cases/update-user-use-case.js"

type UpdateUserHandler = (
  updateUser: UpdateUser
) => RequestHandler<never, LinkuAPI.UpdateUser["ResponseBody"], LinkuAPI.UpdateUser["RequestBody"]>

export const updateUserHandler: UpdateUserHandler = (updateUser) => async (req, res) => {
  ...
  const { ok, data, error } = await updateUser.execute(userId, req.body)
